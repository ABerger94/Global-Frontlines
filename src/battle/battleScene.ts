...
const NVG_SHADER = {
  ...
  fragmentShader: `uniform sampler2D tDiffuse; uniform float amount; uniform float time; varying vec2 vUv;
    ...
    void main(){ vec4 c = texture2D(tDiffuse, vUv); float l = dot(c.rgb, vec3(0.3,0.59,0.11)); l = pow(clamp(l * 6.0, 0.0, 1.5), 0.6);
      float n = rand(vUv) * 0.12; vec3 g = vec3(0.25, 1.0, 0.35) * (l + n);
